...
const text = (max) => {
  ...
  return { type: 'text', max };
};
const integer = (min, max) => {
  if (max === MAX || max === undefined) {
    max = 2147483647;
  }
  if (min === undefined) {
    min = -2147483648;
  ...
  return { type: 'integer', tag: tags.integer, min, max };
};

const rangeOfInteger = (min, max) => {
  if (max === MAX || max === undefined) {
    max = 2147483647;
  }
  if (min === undefined) {
  ...
  return { type: 'rangeOfInteger', tag: tags.rangeOfInteger, min, max };
};
...
const collection = (group, name) => {
  ...
  const defer = Object.keys(group).some((key) => {
...
attributes['Event Notifications'] = {
  ...
  'notify-sequence-number': integer(1, MAX),
...
attributes['Job Description'] = {
  ...
  'output-device-job-state-reasons': setof(keyword),
...
attributes.Operation = {
  ...
  'attributes-natural-language': naturalLanguage,
  compression: keyword,
  'compression-accepted': setof(keyword),
  ...
  'document-uri': uri,
  'fetch-status-code': enumeration,
  'fetch-status-message': text,
  ...
  'notify-subscription-ids': setof(_(integer(1, MAX), rangeOfInteger(1, MAX))),
  'notify-sequence-numbers': setof(_(integer(1, MAX), rangeOfInteger(0, MAX))),
  'notify-wait': boolean,
  'original-requesting-user-name': name,
  'output-device-job-states': setof(enumeration),
...
attributes['Printer Description'] = {
  ...
  'marker-colors': setof(name),
  'marker-high-levels': setof(integer(0, 100)),
  'marker-levels': setof(integer(-3, 100)),
  'marker-low-levels': setof(integer(0, 100)),
  'marker-names': setof(name),
  'marker-types': setof(_(keyword, name)),
